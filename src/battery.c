...
u8 BATTERY_Check()
{
    ...
    if ((warned & BATTERY_LOW) && ms >= next_battery_warning) {
        ...
        MUSIC_PlayValue(MUSIC_BATT_ALARM, battery/10,VOICE_UNIT_VOLT,2);
#else
    ...
    if (battery < Transmitter.batt_critical && ! (warned & BATTERY_CRITICAL)) {
        ...
        Flash_BlockWriteEnable(0);  // Disable writing to all banks of SPIFlash
        warned |= BATTERY_CRITICAL;
        PAGE_ShowLowBattDialog();
    } else if (battery > Transmitter.batt_critical + 200) {
        warned &= ~BATTERY_CRITICAL;
        Flash_BlockWriteEnable(1);  // Disable writing to all banks of SPIFlash
    }
    return warned;
}
