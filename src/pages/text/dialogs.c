...
void PAGE_ShowBindingDialog(u8 update)
{
    ...
    if (dialog && crc != dialogcrc) {
        ...
        current_selected_obj = GUI_GetSelected();
        dialog = GUI_CreateDialog(&gui->dialog, 0, 3, LCD_WIDTH, LCD_HEIGHT-5, NULL, NULL, binding_ok_cb, dtOk, tempstring);
    }
    dialogcrc = crc;
}
...
void PAGE_ShowWarning(const char *title, const char *str)
{
    ...
    current_selected_obj = GUI_GetSelected();
    dialog = GUI_CreateDialog(&gui->dialog, 0, 3, LCD_WIDTH, LCD_HEIGHT-5, NULL, NULL, lowbatt_ok_cb, dtOk, tempstring);
}
...
void PAGE_ShowInvalidStandardMixerDialog(void *guiObj)
{
    ...
    tempstring[sizeof(tempstring) - 1] = 0;
    current_selected_obj = GUI_GetSelected();
    dialog = GUI_CreateDialog(&gui->dialog, 0, 3, LCD_WIDTH, LCD_HEIGHT-5, NULL,
...
void PAGE_ShowResetPermTimerDialog(void *guiObject, void *data)
{
    ...
    if (dialog)
        return;
    dialog = GUI_CreateDialog(&gui->dialog, 1 , 3,  LCD_WIDTH-2, LCD_HEIGHT-5 , NULL , reset_timer_string_cb, reset_permtimer_cb, dtOkCancel, data);
}
