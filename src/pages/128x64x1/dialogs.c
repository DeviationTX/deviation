...
/********************************/
const char *permtimer_string_cb(guiObject_t *obj, void *data)
{
    (void)obj;
    (void)data;
...
void PAGE_ShowResetPermTimerDialog(void *guiObj)
{
    ...
    dlgstr[sizeof(dlgstr) - 1] = 0;
    current_selected_obj = GUI_GetSelected();
    dialog = GUI_CreateDialog(&gui->dialog, 2, 2, LCD_WIDTH - 4, LCD_HEIGHT - 4, NULL, permtimer_string_cb,
            reset_permtimer_cb, dtOkCancel, guiObj);
}
