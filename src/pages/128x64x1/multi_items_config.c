...
void refresh_page_info(u8 ignore_labels)
{
    ...
    case protocol:
        ...
        if (mcp->buttonObj == NULL )
        { // Do not create button more than once
            mcp->buttonObj = GUI_CreateButton((LCD_WIDTH - 32) / 2, y,
                    BUTTON_DEVO10, NULL, 0x0000, NULL, (void *) _tr("Bind"));
...
static void navigate_item(short direction)
{
    ...
    case protocol:
        if (obj == mcp->itemObj[protocol_item]) {
            ...
            if (select == 0)
                select = PROTOCOL_COUNT - 1; // rewind
            else if (select >= PROTOCOL_COUNT)
                select = 1;
