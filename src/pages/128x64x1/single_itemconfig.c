...
void PAGE_SingleItemConfigInit(int page)
{
	config_type = page;
    PAGE_SetModal(0);
    BUTTON_RegisterCallback(&sicp->action,
...
{
	BUTTON_UnregisterCallback(&sicp->action);
}

const char *value_cb(guiObject_t *obj, const void *data){
	(void)obj;
	(void)data;
	return (const char *)sicp->item_name[sicp->selected];
}
...
void navigate_item(short direction)
{
	// navigate among options in a object
	strncpy(sicp->tmpstr, (const char *)sicp->item_name[sicp->selected], sizeof(sicp->tmpstr)); // don't let the keyboard change target directly as we might press Ext to cancel the change
	if (config_type == modelName) { // show keyboard
...
void refresh_itemName() {
	...
	switch (config_type) {
	case language:
		strcpy((char *)sicp->item_name[0], (const char *)"English");
		item_count = 1;
				...
				if (type == 1 && strncasecmp(filename, "lang", 4) == 0) {
					...
					if (fh) {
						...
						if(strlen(sicp->tmpstr) && sicp->tmpstr[0] != ':') {
							sicp->tmpstr[len-1] = '\0';
							strcpy((char *)sicp->item_name[item_count], (const char *)sicp->tmpstr);
						}
					}
					item_count++;
	...
	case stickMode:
		...
		for (u8 i = 0; i < item_count; i++) {
			sprintf((char *)sicp->item_name[i], _tr("Mode %d"), i+1);
		}
	...
	case powerAmplifier:
		...
		for (u8 i = 0; i < item_count; i++) {
			sprintf((char *)sicp->item_name[i], RADIO_TX_POWER_VAL[i]);
		}
		sicp->selected = Model.tx_power;
		break;
	case swashType:
		item_count = SWASH_TYPE_LAST;
		strcpy((char *)sicp->item_name[0], "1 Servo");
		for (u8 i = 0; i < item_count; i++) {
			switch(i) {
				case SWASH_TYPE_NONE:
					strcpy((char *)sicp->item_name[i], "1 Servo"); //return _tr("None");
					break;
				case SWASH_TYPE_120:
					strcpy((char *)sicp->item_name[i], "120");
					break;
				case SWASH_TYPE_120X:
					strcpy((char *)sicp->item_name[i], "120x");
					break;
				case SWASH_TYPE_140:
					strcpy((char *)sicp->item_name[i], "140");
					break;
				case SWASH_TYPE_90:
					strcpy((char *)sicp->item_name[i], "90");
					break;
			}
		}
		...
		break;
	case modelName:
		strncpy((char *)sicp->item_name[0], (const char *)Model.name, sizeof(sicp->item_name[0]));
		break;
	default:
		break;
	}
