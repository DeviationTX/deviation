...
static void _show_page()
{
	...
	firstObj = GUI_CreateLabelBox(&gui2->musicshutdbl, 16, row, 0, 0, &DEFAULT_FONT, NULL, NULL, _tr("PowerOff alert"));
        ...
        GUI_CreateTextSelect(&gui2->temp, 112, row, TEXTSELECT_96, NULL, units_cb, (void *)1L);
        row += space;
        GUI_CreateLabelBox(&gui2->lengthlbl, 16, row, 0, 0, &DEFAULT_FONT, NULL, NULL, _tr("Length:"));
        GUI_CreateTextSelect(&gui2->length, 112, row, TEXTSELECT_96, NULL, units_cb, (void *)0L);
    } else if (page_num == 2) {
