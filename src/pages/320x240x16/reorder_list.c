...
void PAGE_ShowReorderList(u8 *list, u8 count, u8 selected, u8 max_allowed, const char *(*text_cb)(u8 idx), void(*return_page)(u8 *))
{
    ...
    if (max_allowed) {
        GUI_CreateButton(&gui->insert, 8 + REORD_XOFFSET, 140, BUTTON_96x16, _show_button_cb, press_button_cb, (void *)INSERT);
        GUI_CreateButton(&gui->remove, 8 + REORD_XOFFSET, 160, BUTTON_96x16, _show_button_cb, press_button_cb, (void *)REMOVE);
    }
