...
void __attribute__((__used__)) JREXT_DMA_ISR(void)
{
    timer_disable_counter(SSER_TX_TIM);
    nvic_disable_irq(NVIC_DMA2_STREAM1_IRQ);
    dma_clear_interrupt_flags(JREXT_DMA.dma, JREXT_DMA.stream, DMA_TCIF);
...
#ifdef JREXT_NO_DMA
void __attribute__((__used__)) tim8_cc_isr(void)
{
	if (timer_get_flag(SSER_TX_TIM, TIM_SR_CC1IF)) {

		/* Clear compare interrupt flag. */
		timer_clear_flag(SSER_TX_TIM, TIM_SR_CC1IF);
		timer_set_period(SSER_TX_TIM, *jrptr);
                jrptr++;
