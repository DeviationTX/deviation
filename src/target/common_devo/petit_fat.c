...
#include "petit_fat.h"		/* Petit FatFs configurations and declarations */
#include "petit_io.h"		/* Declarations of low level disk I/O functions */
...
)
{
	...
	case FS_FAT12 :
		...
		ofs = bc % SECTOR_SIZE; bc /= SECTOR_SIZE;
		if (ofs != (SECTOR_SIZE - 1)) {
			if (disk_readp(buf, fs->fatbase + bc, ofs, 2)) break;
		} else {
			if (disk_readp(buf, fs->fatbase + bc, (SECTOR_SIZE - 1), 1)) break;
...
#endif
	case FS_FAT16 :
		if (disk_readp(buf, fs->fatbase + clst / (SECTOR_SIZE / 2), (WORD)(((WORD)clst % (SECTOR_SIZE / 2)) * 2), 2)) break;
		return LD_WORD(buf);
#if _FS_FAT32
	case FS_FAT32 :
		if (disk_readp(buf, fs->fatbase + clst / (SECTOR_SIZE / 4), (WORD)(((WORD)clst % (SECTOR_SIZE / 4)) * 4), 4)) break;
...
)
{
	...
	if (!(i % DIR_ENTRIES_PER_SECTOR)) {		/* Sector changed? */
		dj->sect++;			/* Next sector */
		...
		if (dj->clust == 0) {	/* Static table */
			...
			if (((i / DIR_ENTRIES_PER_SECTOR) & (fs->csize-1)) == 0) {	/* Cluster changed? */
...
)
{
	...
	do {
		res = disk_readp(dir, dj->sect, (WORD)((dj->index % DIR_ENTRIES_PER_SECTOR) * 32), 32)	/* Read an entry */
			? FR_DISK_ERR : FR_OK;
		if (res != FR_OK) break;
...
)
{
	...
	mclst = (tsect						/* Last cluster# + 1 */
		- LD_WORD(buf+BPB_RsvdSecCnt-13) - fsize - fs->n_rootdir / DIR_ENTRIES_PER_SECTOR
		) / fs->csize + 2;
	...
	fs->database = fs->fatbase + fsize + fs->n_rootdir / DIR_ENTRIES_PER_SECTOR;	/* Data start sector (lba) */
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		if ((fs->fptr % SECTOR_SIZE) == 0) {				/* On the sector boundary? */
			cs = (BYTE)(fs->fptr / SECTOR_SIZE & (fs->csize - 1));	/* Sector offset in the cluster */
...
)
{
	...
	*bw = 0;
	...
	while (btw)	{									/* Repeat until all data transferred */
		if (((WORD)fs->fptr % SECTOR_SIZE) == 0) {			/* On the sector boundary? */
			cs = (BYTE)((fs->fptr / SECTOR_SIZE) & (fs->csize - 1));	/* Sector offset in the cluster */
			if (!cs) {								/* On the cluster boundary? */
				clst = (fs->fptr == 0) ?			/* On the top of the file? */
					fs->org_clust : get_fat(fs->curr_clust);
		...
		wcnt = SECTOR_SIZE - ((WORD)fs->fptr % SECTOR_SIZE);		/* Number of bytes to write to the sector */
		if (wcnt > btw) wcnt = btw;
		if (disk_writep(p, wcnt)) goto fw_abort;	/* Send data to the sector */
		fs->fptr += wcnt; p += wcnt;				/* Update pointers and counters */
		btw -= wcnt; *bw += wcnt;
		if (((WORD)fs->fptr % SECTOR_SIZE) == 0) {
...
)
{
	...
	if (ofs > 0) {
		...
		fs->dsect = sect + (fs->fptr / SECTOR_SIZE & (fs->csize - 1));
	}
