...
{
    if(bind) {
        memcpy(packet, (u8*)"\x38\x55\x32\x3f\x42\x47\x00\x00\x00\x00\x00\x00\x00\xaa\xf6", PAYLOAD_SIZE);
    }
    else {
        packet[0] = 0x38;
        packet[1] = 0x01;
        packet[2] = scale_channel(CHANNEL1, 200, 0);
        packet[3] = scale_channel(CHANNEL2, 0, 200);
        packet[4] = scale_channel(CHANNEL4, 200, 0);
        packet[5] = scale_channel(CHANNEL3, 0, 200);
        packet[6] = 0xaa;
